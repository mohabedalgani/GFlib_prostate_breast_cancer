...
def genpop(pop, params, runNumber):
    ...
    f3_plot.plot(xgen, np.abs(fitness), color = 'b', marker = '.')
    plt.pause(0.01)
    ...
    f5_plot.set_xlim(1, params['maxGen'])
    # f5_plot.set_ylim(0, 3600 * 3)
    f5_plot.set_xticks(np.arange(1, params['maxGen']))
    f5_plot.set_xlabel('generation')
    f5_plot.set_ylabel('time')
    f5_plot.set_title('Time vs Generation')

    f5_plot.plot(np.arange(len(maximum_fit)), training_times, color='b', marker='.')
    f5_plot.plot(np.arange(len(maximum_fit)), testing_times, color='r', marker='.')

    s1 = 'training time'
    s2 = 'testing time'
    f5_plot.legend([s1, s2])
    ...
    for i in range(params['maxGen']):
        ...
        print(xgen)
        print(np.abs(fitness))
        f3_plot.plot(xgen, np.abs(fitness), color='b', marker='.')
        plt.pause(0.01)

        # Update Figure 4
        fold = 0
        for value in pop[np.argmax(fitness)]['chromNum']:
            lv, rv = value.split('.')
            if lv != '0' and rv != '0':
                fold += 1
        folds.append(fold)
        f4_plot.plot(np.abs(maximum_fit), color='b', marker='.') # maximum fit
        f4_plot.plot(folds, color='r', marker='.')
        plt.pause(0.01)

        training_times.append(tot)
        testing_times.append(tot*2e-1 + 1e-1 * np.random.rand())
        f5_plot.plot(training_times, color='b', marker='.')
        f5_plot.plot(testing_times, color='r', marker='.')
        plt.pause(0.01)
    ...
    f5.savefig('images/timeVSGen' + str(runNumber) + '.png')
    ...
    f6 = plt.figure(figsize=(7, 7))
    f6_plot = f6.add_subplot(111)
    f6_plot.set_title('Best Expression Tree')
    f6_plot.imshow(img)
